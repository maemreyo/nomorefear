"""
    ...
    |⚙️ HOW IT WORKS?
    @Initialization
    ==> The algorithm divides the input array into two parts: The sorted sub-array and the unsorted sub-array
    ==> Initially, the sorted sub-array is empty, and the unsorted sub-array contains all elements of the input array

    @Selection
    ==> The algorithm repeatedly selects the smallest element from the unsorted sub-array and swap it with the first
    element of the unsorted sub-array. This effectively moves the smallest element to its correct position in the sorted
    sub-array

    @Expansion of Sorted Sub-Arrays
    ==> After each selection step, the size of the sorted sub-array increases by 1, and the size of the unsorted
    sub-array decreases by 1

    @Termination
    ==> The algorithm terminates when the entire array is sorted, i,e., when the unsorted sub-array becomes empty
