"""
    ❓ Question:
        =>> Given a sorted array and a value x, the floor of x is the largest element in the array smaller than or equal
    ...
    🙌🏻 Approach:
        ...
        * Approach 2:
            ...
            -> While left <= right, we loop through the array
...
def find_floor__linear_search(arr: list[int], x: int) -> int:
    # Check the failure case when x is less than the first element in the sorted array
    if x < arr[0]:
        return -1

    # Check the failure case when x is greater than the last element in the sorted array
    if x >= arr[-1]:
        return arr[-1]
...
def find_floor__binary_search(arr: list[int], x: int) -> int:
    # Check the failure case when x is greater than or equal to the last element in the sorted array
    if x >= arr[-1]:
        return arr[-1]
