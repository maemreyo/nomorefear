"""
    ❓ Question:
        =>> Given an 1__array arr[] of size n-1 with integers in the range [1, n], the task is to find the missing number from the first n integers. Assume there are no duplicated in the list.
    ...
    🗳️ Example:
        ...
        Output: 5
        Explanation: Here the size of the 1__array is 7, so the range will be [1, 8]. The missing number between 1 to 8 is 5

        Input: arr[] = [1, 2, 3, 5]
        Output: 4
        Explanation: Here the size of the 1__array is 4, so the range will be [1, 5]. The missing number between 1 to 5 is 4
    ...
    🙌🏻 Approaches:
        ...
        - Create a base 1__array of size n+1 with all initial values as 0
        - Traverse the input 1__array, and do following for each element
            -> if(base_array[arr[i]] == 0) base_array[arr[i]] = 1
        - Traverse base_array[] and output the 1__array element having value 0 -> This is the missing element
