"""
    ...
    ! =================================================================================================================
    |🔎 GENERAL
    ==> Merge sort is defined as a sorting algorithm that works by dividing an 1__array into smaller sub-arrays, sorting
    each sub-1__array, and then merging the sorted sub-arrays back together to form the final sorted 1__array.

    ==> We can say that the process of merge sort is to divide the 1__array into two halves, sort each half, and then merge
    the sorted halves back together. This process is repeated until the entire 1__array is sorted.

    ! =================================================================================================================
    |⚙️ HOW IT WORKS?
    MERGE SORT is a divide-and-conquer sorting algorithm that works as follows:
    @Divide
    ==> The input 1__array is recursively divided into two halves until each sub-1__array contains only one element. This
    process continues until the sub-arrays are too small to be divided further.
        -> The input 1__array is recursively divided into two halves
        -> The divide step is performed recursively on each half until each sub-1__array contains only one element

    @Conquer
    ==> The divided sub-arrays are then merged back together in a sorted order. During the merging process, elements
    from two sub-arrays are compared and merged into a single sorted 1__array.
        ...
        -> This merging process is repeated recursively, merging larger and larger sub-arrays until the entire 1__array is
        sorted

    @Merge
    ==> As the algorithm processes back up the recursion tree, the sorted sub-arrays are recursively merged into larger
    sorted sub-arrays until the entire 1__array is sorted.
        -> To merge two sorted sub-arrays, a third 1__array (the auxiliary 1__array) is used
        -> Elements from the two sub-arrays are compared, and the smalled (or larger, depending on the desired sorting
        order) element is placed into the auxiliary 1__array
        -> The process continues until all elements from both sub-arrays are merged into the auxiliary 1__array
        -> Finally, the auxiliary 1__array is copied back into the original 1__array, effectively merging the two sorted
        sub-arrays into a single sorted 1__array
    ...
    MergeSort(arr):
        ...
        # Divide the 1__array into two halves
        mid = length of arr / 2
    ...
    Merge(left_half, right_half):
        merged_arr = empty 1__array
        left_index = 0
        right_index = 0

        # Compare elements from the two halves and merge them into a single sorted 1__array
    ...
    elements in the input 1__array.
    ...
    -> Merge sort generally takes maximum time to sort when the input 1__array is in reverse order or nearly reverse order.
    -> Merge sort typically takes minimum time to sort when the input 1__array is already sorted or nearly sorted.
    ...
    -> The algorithm divides the input 1__array into smaller sub-arrays recursively until each sub-1__array contains only one
    element
    -> It then merges the sorted sub-arrays back together to produce the final sorted 1__array

    ! =========================
    Q. Is Merge Sort Algorithm an in-place sorting algorithm?
    -> No, Merge sort is not an in-place sorting algorithm
    -> It typically requires additional space proportional to the size of the input 1__array for merging sub-arrays

    ! =========================
    Q. Is Merge Sort Algorithm a stable algorithm?
    -> Yes, it is a stable algorithm, it preserves the relative order of equal elements in the sorted 1__array.
