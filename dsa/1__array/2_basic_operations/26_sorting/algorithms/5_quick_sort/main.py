"""
    ...
    and partitions the given 1__array around the picked pivot by placing the pivot in its correct position in the sorted
    1__array.

    ! =================================================================================================================
    |⚙️ HOW IT WORKS?
    @Choose a Pivot
    -> Select a pivot element from the 1__array.
    ...
    @Partitioning
    -> Reorder the 1__array so that all elements less than the pivot come before it, and all elements greater than the
    pivot come after it.
    ...
    -> This process continues until the  base case is reached, where the sub-1__array has zero or one element (which is
    already sorted)

    @Combine Sorted Sub-arrays
    -> As the recursion unwinds, the sorted sub-arrays are combined to produce the final sorted 1__array. Since the pivot
    ...
    Partition(arr, low, high):
        ...
        # Partition the 1__array around the pivot
    ...
    -> Quick Sort takes the maximum time to sort if the chosen pivot divides the 1__array into extremely unbalanced
    partitions, such as the smallest or largest element as the pivot.
    -> It takes minimum time O(n) when the chosen pivot consistently divides the 1__array into roughly equal-sized
    partitions.
    ...
    result, the relative order of equal elements in the input 1__array may not be preserved in the sorted output.
    ...
    -> It is commonly used in various scenarios where efficient sorting of large arrays is required:
        ...
        -> Parallel processing
            ...
            of the 1__array concurrently. This parallelization can lead to significant performance gains on multi-core
...
def quick_sort(arr):
    ...
    def quick_sort_recursive(arr, low, high):
        if low < high:
            # Partition the 1__array and get the pivot index
            pivot_index = partition(arr, low, high)
